clear all
b=0.9;%i_rf
hwhm=0.25;%HWHM of peak in resistance at Shapiro step 4
c=0:0.01:1.3; %i_dc
x=length(c);
...
ht=0:0.5:30;%Height of peak in resistance at Shapiro step 4
s=length(ht);
...
for j=1:s
    ...
    for m=1:x
        ...
        Res=@(Ibias)(33+((ht(j))*((hwhm.^2)/((hwhm.^2)+((Ibias-c_even(2)).^2)))));
...
hist3(his,'Ctrs',{0:0.5:30 -0.2:0.2:20.2},'CDataMode','auto','EdgeColor','interp');
xlabel('Peak Height $(\Omega)$', 'interpreter', 'latex');
